...
func TestVulnGetProductsByCategory(t *testing.T) {
	...
	mock.ExpectQuery("SELECT \\* FROM products WHERE category = 'Electronics'").
		WillReturnRows(rows)

	products, err := VulnGetProductsByCategory("Electronics")
...
func TestVulnGetProductsByCategory_ErrorInQuery(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.NoError(t, err)
	defer db.Close()
...
func TestSecureGetProductsByCategory_ErrorInPrepare(t *testing.T) {
	db, mock, err := sqlmock.New()
	assert.NoError(t, err)
	defer db.Close()
